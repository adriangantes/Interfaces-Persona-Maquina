...
void main(){
  ...
  tz.setLocalLocation(tz.getLocation('Europe/Madrid')); // Ajusta a tu zona horaria
...
class LoginPage extends StatelessWidget {
  ...
  final TextInputFormatter _idFormatter = FilteringTextInputFormatter.allow(RegExp(r'[\d-]'));

  @override
  Widget build(BuildContext context){
    double textSize;
    double screenWidth = MediaQuery.of(context).size.width; //sacar el ancho de pantalla
    double screenHeight = MediaQuery.of(context).size.height; //sacar la altura de la pantalla
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Login", style: TextStyle(fontSize: textSize), textAlign: TextAlign.center,), //alinear el texto centrado en el widget
        ...
        child: Column(
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(labelText: "Enter your patient ID", labelStyle: TextStyle(fontSize: textSize)),
              keyboardType: TextInputType.text, //teclado con texto para permitir signos - (por defecto en el smartwatch no sale en el numerico
              style: TextStyle(fontSize: textSize), //ajuste dinamico del texto
              inputFormatters: [_idFormatter],  // Usamos el TextInputFormatter personalizado
            ),
              ...
              child: ElevatedButton(
                onPressed: () async {
                  if (idController.text.isNotEmpty) {
                    String patientIdText = idController.text;
                    final idRegex = RegExp(r'^\d{3}-\d{2}-\d{4}$'); // Formato "###-##-####"

                    if (idRegex.hasMatch(patientIdText)) {
                      try {
                        // Llamamos a la función asincrónica para obtener el patientId
                        int patientId = await getIdPatientFromText(patientIdText);
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text("Error: ${e.toString()}", style: TextStyle(fontSize: textSize-3), textAlign: TextAlign.center,),
                          ),
                        );
                      }
                    } else {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text("ID invalid. Format ###-##-####.",style: TextStyle(fontSize: textSize -3), textAlign: TextAlign.center, ),
                        ),
                      );
                    }
                  } else {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text("Please, ID cannot be empty.", style: TextStyle(fontSize: textSize-3), textAlign: TextAlign.center,),
                      ),
                    );
                  }
                },
                child: Text("Login", style: TextStyle(fontSize: textSize),textAlign: TextAlign.center,),
              ),
            ),
          ],
        ),
...
class MedicineListPage extends StatefulWidget {
  ...
  const MedicineListPage({required this.patientId, required this.textSize, required this.padding, super.key});
  @override
    ...
    return Scaffold(
      appBar: AppBar(
        title: FutureBuilder<String>(
          future: namePatient(widget.patientId), // Llama al Future que obtiene el nombre
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            context.watch<MedicineModel>().errorMessage != null //por si hay algun mensaje de error mostrarlo
              ?Padding(
                padding: EdgeInsets.all(widget.padding),
                child: ErrorMessage(
                  message: context.watch<MedicineModel>().errorMessage!, textSize: widget.textSize, padding: widget.padding,), )
              : ListView.builder(
                  shrinkWrap: true,
                  ...
                  itemBuilder: (context, index) {
                    return Padding(
                      padding: EdgeInsets.all(widget.padding),
                      child: MedicineTitle(medicine: context.watch<MedicineModel>().medicines[index], textSize: widget.textSize, padding: widget.padding,));
                  },
              ),
              if (context.watch<MedicineModel>().isLoading)
                ...
                context.watch<MedicineModel>().hasNoMedicines
                  ? Padding(
                      padding: EdgeInsets.all(widget.padding),
                      child: Center(child: Text("This patient has no medications", style: TextStyle(fontSize: widget.textSize),),), )
                  : Container(),
          ],
        ),
      ),
...
class MedicineDetailsPage extends StatefulWidget {
  ...
  final double textSize;
  final double padding;
  const MedicineDetailsPage(this.medicine, this.textSize, this.padding, {super.key});
...
class _MedicineDetailsPage extends State<MedicineDetailsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Medication details", style: TextStyle(fontSize: widget.textSize),),
        backgroundColor: Theme.of(context).primaryColor,
        foregroundColor: Colors.white,
      ),
      body: FutureBuilder(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasError) {
              return ErrorMessage(message: snapshot.error.toString(), textSize: widget.textSize, padding: widget.padding,);
            } else {
              if (!snapshot.hasData) {
                                        ...
                                        Navigator.push(
                                          ...
                                          MaterialPageRoute(
                                            builder: (context) => PosologiesListPage(
                                                ...
                                                medicationId: widget.medicine.id, textSize: widget.textSize, padding: widget.padding,),
                                          ),
                                        );
                                      },
                                      child: Text("Posologies", style: TextStyle(fontSize: widget.textSize -2),),
                                    ),
                                    SizedBox(width: 8,), //espacio entre os botones
                                    ElevatedButton(
                                      onPressed: () {
                                        ...
                                        Navigator.push(
                                          ...
                                          MaterialPageRoute(
                                            builder: (context) => IntakesListPage(
                                                patientId: widget.medicine.patient_id,
                                                medicationId: widget.medicine.id, padding: widget.padding, textSize: widget.textSize,),
                                          ),
                                        );
                                      },
                                      child: Text("Intakes", style: TextStyle(fontSize: widget.textSize -2),),
                                    )
                                  ],
                                ))
                          ],
...
class MedicineDetailsTile extends StatelessWidget {
  const MedicineDetailsTile(
      {super.key, required this.title, required this.content, required this.textSize, required this.padding});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(title, style: TextStyle(fontSize: textSize), overflow: TextOverflow.ellipsis,),
          Row(
            ...
            children: [
              ...
              child: SingleChildScrollView(
                scrollDirection: Axis.horizontal,
                            child: Text(
                              content,
                              style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontSize: textSize),
...
class PosologiesListPage extends StatefulWidget {
  ...
  const PosologiesListPage({required this.patientId, required this.medicationId, required this.textSize, required this.padding, super.key});
...
class _PosologiesListPageState extends State<PosologiesListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: FutureBuilder<String>(
          future: namePatient(widget.patientId), // Llama al Future aquí
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
        ...
        child: Column(
          ...
          children: [
            context.watch<PosologieModel>().errorMessage != null
                ? ErrorMessage(
                    message: context.watch<PosologieModel>().errorMessage!, textSize: widget.textSize, padding: widget.padding,)
                : ListView.builder(
              shrinkWrap: true,
              physics: const NeverScrollableScrollPhysics(),
              itemCount: context.watch<PosologieModel>().posologies.length,
              itemBuilder: (context, index) {
                return PosologieTitle(posologie: context.watch<PosologieModel>().posologies[index], textSize: widget.textSize, padding: widget.padding,);
              },
            ),
            ...
            context.watch<PosologieModel>().hasNoPosologies
                ? Center(child: Text("This patient for this medication has not posologies", style: TextStyle(fontSize: widget.textSize),))
                : Container(),
          ],
        ),
      ),
...
class IntakesListPage extends StatefulWidget {
  ...
  const IntakesListPage(
      {required this.patientId, required this.medicationId, required this.padding, required this.textSize, super.key});
...
class _IntakesListPageState extends State<IntakesListPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final isSmallScreen = screenSize.width < 300; //si el ancho de la pantalla es menor a 300
    final buttonSize = isSmallScreen ? 26.0 : 58.0; //tamaño del boton si es o no pantalla pequeña
    return Scaffold(
      appBar: AppBar(
        title: FutureBuilder<String>(
          future: namePatient(widget.patientId), // Llama al Future aquí
        ...
        child: Column(
          ...
          children: [
            context.watch<IntakeModel>().errorMessage != null
                ? ErrorMessage(
                    message: context.watch<IntakeModel>().errorMessage!, textSize: widget.textSize, padding: widget.padding,)
                : ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    ...
                    itemBuilder: (context, index) {
                      return IntakeTitle(
                          intake: context.watch<IntakeModel>().intakes[index], textSize: widget.textSize, padding: widget.padding,);
                    },
                  ),
            ...
            context.watch<IntakeModel>().hasNoIntakes
                ? Center(
                    child: Text(
                        "This patient for this medication has not intakes", style: TextStyle(fontSize: widget.textSize),))
                : Container(),
          ],
        ),
      ),
      floatingActionButtonLocation: FloatingActionButtonLocation.centerFloat, //centrar el boton flotante
      floatingActionButton: SizedBox(
        ...
        height: buttonSize, //alto del boton
          child: FloatingActionButton(
            ...
            backgroundColor: Theme.of(context).primaryColor, //color del fondo que lo obtenga color prinicpal de la app
          ),
      ),
    );
  }
...
class IntakeAddPage extends StatefulWidget {
  ...
  // no puede ser const porque no siempre que se crea es igual
   IntakeAddPage(
      {required this.patientId, required this.medicationId, required this.textSize, required this.padding,super.key});
                                ...
                                itemBuilder: (context, minute) {
                                  return ListTile(
                                    ...
                                    onTap: () {
                                      ...
                                      Navigator.pop(context); // Cierra el selector de minutos
                                      Navigator.pop(context); // Cierra el selector de horas
                                    },
                                  );
                                },
                              ),
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // Fecha
            Text("Date", style: TextStyle(fontSize: widget.textSize),),
            Row(
              children: [
                Expanded(
                  child: InkWell(
                    child: Text(
                      ...
                      style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontSize: widget.textSize),
                      textAlign: TextAlign.center, //centrar el valor del campo
                    ),
                    onTap: () => isSmartwatch ? _selectDateSmartwatch() : _selectDate(),
                  ),
                ),
              ],
            ),
            ...
            // Hora
             Text("Time", style: TextStyle(fontSize: widget.textSize),),
            Row(
              children: [
                Expanded(
                  child: InkWell(
                    ...
                    onTap: () => isSmartwatch ? _selectTimeSmartwatch() : _selectTime(),
                  ),
                )
              ],
            ),
            ...
            // Boton para guardar
            Padding(
              padding: EdgeInsets.symmetric(vertical: widget.padding), //centrar el boton
              child: ElevatedButton(
                onPressed: () async {
                  try {
                    await context.read<IntakeModel>().addIntake(
                        widget.patientId, widget.medicationId,
                        widget.selectedDate);
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        behavior: SnackBarBehavior.floating, //para que no este pegado abajo el mensaje de error y permita flotar
                        content: Text('Error: ${error.toString()}', style: TextStyle(fontSize: widget.textSize-3), textAlign: TextAlign.center,),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
                },
                child: Text("Save", style: TextStyle(fontSize: widget.textSize),),
              ),
            ),
          ],
        ),
    ...
    return Padding(
      ...
      child: Text(
        ...
        style: Theme.of(context)
            .textTheme
            .bodyMedium!
            .copyWith(color: Color.fromARGB(255, 173, 68, 68),
          fontSize: textSize, //tamaño del texto por si es un reloj, movil etc
        ),
      ),
    );
  }
